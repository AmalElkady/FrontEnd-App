...
import data from "../app/contact/data/contactList";
  ...
  switch (action.type) {
    ...
    case ON_DELETE_CONTACT: {
      return {
        ...
        showMessage: true,
        allContact: state.allContact.filter(
          contact => contact.id !== action.payload.id
        ),
        contactList: state.allContact.filter(
          contact => contact.id !== action.payload.id
        )
      };
    }
    ...
    case FILTER_CONTACT: {
      ...
      if (action.payload === "") {
        ...
        const filterContact = state.allContact.filter(
          contact =>
    ...
    case ON_ALL_CONTACT_SELECT: {
      ...
      let contactList = [];
      if (selectAll) {
        contactList = state.allContact.map(contact =>
          contact
            ? {
                ...contact,
                selected: true
