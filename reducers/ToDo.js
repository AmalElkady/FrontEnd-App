...
import todoConversation from "../app/todo/data/todoConversation";
import toDos from "../app/todo/data/todo";
import {
  ...
  switch (action.type) {
    ...
    case ON_LABEL_UPDATE: {
      if (action.payload.data.labels.includes(action.payload.label.id)) {
        action.payload.data.labels.splice(
          action.payload.data.labels.indexOf(action.payload.label.id),
          1
        );
      } else {
        action.payload.data.labels = action.payload.data.labels.concat(
          action.payload.label.id
        );
      }
      const toDos = state.allToDos.map(todo => {
    ...
    case GET_NAV_LABELS: {
      const filterMails = state.allToDos.filter(todo =>
        todo.labels.includes(action.payload.id)
      );
      return {
        ...state,
        loader: true,
    ...
    case ON_TODO_SELECT: {
      let conversationList = state.todoConversation.find(
