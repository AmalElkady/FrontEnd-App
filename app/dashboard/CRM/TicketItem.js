  ...
  return (
    <Aux>
      <span className="jr-nonhover">
        <i className={`zmdi zmdi-circle jr-fs-sm text-${statusData.color}`} />
      </span>
      <span
    ...
    >
      ...
      <div className="media-body jr-task-item-content">
        <div className="jr-task-item-content-left">
          ...
          <p key={id} className="text-grey jr-fs-sm mb-0">
            {description}
          </p>
        </div>
        <div className="jr-task-item-content-right">{getStatus(status)}</div>
      </div>
    </div>
  );
};
